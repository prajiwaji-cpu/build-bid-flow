...
// Add this section to your Dashboard.tsx right after the header and before the stats cards

{/* DEBUG SECTION - Add this after the main title but before stats cards */}
<div className="mb-6 p-4 bg-muted/30 rounded-lg border-2 border-dashed border-muted-foreground/30">
  <h3 className="text-sm font-medium mb-2 text-muted-foreground">Debug & Testing Tools</h3>
  <div className="flex gap-2 flex-wrap">
    
    {/* Test with the first available quote */}
    {quotes.length > 0 && (
      <>
        <Button 
          variant="outline" 
          ...
          onClick={async () => {
            try {
              await quotesService.inspectFieldStructures(quotes[0].id);
              toast({ 
                title: "Field Inspection Complete", 
                description: "Check console for detailed field structure analysis" 
              });
            } catch (err) {
              toast({ 
        ...
        <Button 
          ...
          onClick={async () => {
            try {
              await quotesService.runComprehensiveDiagnostics(quotes[0].id);
              toast({ 
                title: "Diagnostics Complete", 
                description: "Check console for detailed diagnostic results" 
              });
            } catch (err) {
              toast({ 
        ...
        <Button 
          ...
          onClick={async () => {
            try {
              const success = await quotesService.testSafeFieldUpdate(quotes[0].id);
              toast({ 
                title: success ? "Test Successful!" : "Test Failed", 
                description: success ? "Basic updates are working" : "Check console for details",
  ...
  <p className="text-xs text-muted-foreground mt-2">
    These buttons help diagnose API update issues. Results will appear in browser console.
    {quotes.length > 0 && ` Testing with quote ${quotes[0].id} (${quotes[0].clientName})`}
  </p>
</div>
